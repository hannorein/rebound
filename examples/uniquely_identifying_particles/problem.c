...
int main(int argc, char* argv[]){
    ...
    r->particles[0].hash = reb_get_particle_hash(r, "Sun"); // We add a hash to particles[0] corresponding to the name "Sun"
     ...
     * names, you can also have the simulation assign particles a unique hash by passing get_particle_hash a NULL string.:
     */

    for (int i=1;i<10;i++){
		struct reb_particle p = {0};
		reb_add(r, p); 
        r->particles[i].hash = reb_get_particle_hash(r, NULL); // this returns consecutive hashes, starting from a random number.
	}
