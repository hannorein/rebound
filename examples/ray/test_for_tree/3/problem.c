...
static struct reb_simulation* setup_close_component_simulation(void){
    ...
    add_circular_particle(r, m, 0.004);  // 2: close to 1, not close to 3
    add_circular_particle(r, m, -0.004); // 3: close to 1, not close to 2
...
static int assert_123_component_shape(const struct hj_node* const node){
    ...
    for (int i=0; i<node->N_children; i++){
        ...
        if (is_leaf_particle(child, 2)){
            has_leaf_2 = 1;
            continue;
        }
        if (is_leaf_particle(child, 3)){
            has_leaf_3 = 1;
            continue;
        }
        ...
        if (mask == (BIT(2) | BIT(3))){
            return fail("particles 2 and 3 should not form a subtree when they are not close");
        }
        if (mask == BIT(2)){
            return fail("particle 2 should be a leaf node");
        }else if (mask == BIT(3)){
            return fail("particle 3 should be a leaf node");
        }else{
            return fail("component {1,2,3} has an unexpected child");
        }
    }
    ...
    if (!has_leaf_2 || !has_leaf_3){
        return fail("particles 2 and 3 should be direct leaf nodes");
    }
    return 1;
}
...
static int assert_45_component_shape(const struct hj_node* const node){
    ...
    for (int i=0; i<node->N_children; i++){
        const struct hj_node* const child = &node->children[i];
        if (is_leaf_particle(child, 4)){
            has_leaf_4 = 1;
        }else if (is_leaf_particle(child, 5)){
            has_leaf_5 = 1;
        }else{
            return fail("component {4,5} has an unexpected child");
        }
    }

    if (!has_leaf_4 || !has_leaf_5){
        return fail("component {4,5} should contain leaf particles 4 and 5");
    }
    return 1;
}
...
int main(void){
    ...
    puts("reb_integrator_whfast_hj_build_tree partial-close tree test passed.");
    return 0;
}
