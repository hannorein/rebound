...
double calculate_viscosity_coll(){
	if (t<t_start || collisions_plog==0){
		return 0;
	}
	double mtot = 0;
	for (int i=0;i<N;i++){
	...
	return 2./(3.*OMEGA*mtot*(t-t_start))*collisions_plog;
}
...
void output_everything(){
	...
	fprintf(of,"%e\t",nutrans_xx_tot/(double)nuN);							//3
	fprintf(of,"%e\t",nutrans_yy_tot/(double)nuN);							//4
	fprintf(of,"%e\t",nutrans_xy_tot/(double)nuN);							//5
	fprintf(of,"%e\t",veldisp_sum   /(double)veldisp_N);						//6	total veldisp^2
	fprintf(of,"%e\t",nucoll_tot);									//7
	...
	fprintf(of,"%d\t",0);										//14	H scale height
