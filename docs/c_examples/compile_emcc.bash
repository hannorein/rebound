...
do
    ...
    if [ $mpi_enabled -eq 0 ] && [ $openmp_enabled -eq 0 ]; then
        ...
        if [ $server_used -eq 0 ]; then
            ...
            emcc -O$OPTIMI -Isrc/ src/*.c $dir/problem.c -DSERVERHIDEWARNING -DOPENGL=1 -sSTACK_SIZE=655360 -s USE_GLFW=3 -sGL_ENABLE_GET_PROC_ADDRESS=1 -s FULL_ES3=1 -sASYNCIFY -sEXPORTED_RUNTIME_METHODS="callMain" --shell-file web_client/shell_rebound_webgl.html -o $READTHEDOCS_OUTPUT/html/emscripten_c_$dir/index.html || exit 1
        fi
        echo "Done. "
    else
        echo "Skipping."
