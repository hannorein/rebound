    ...
    def test_all_1st_order_full(self):
        ...
        for params in paramslist:
            for vi,v in enumerate(vlist):
                ...
                prec = 1e-5
                self.assertLess(abs(p_fd.x -vp.x ), prec, msg="Problem with %s derivative."%v)
                self.assertLess(abs(p_fd.y -vp.y ), prec, msg="Problem with %s derivative."%v)
                self.assertLess(abs(p_fd.z -vp.z ), prec, msg="Problem with %s derivative."%v)
                self.assertLess(abs(p_fd.vx-vp.vx), prec, msg="Problem with %s derivative."%v)
                self.assertLess(abs(p_fd.vy-vp.vy), prec, msg="Problem with %s derivative."%v)
                self.assertLess(abs(p_fd.vz-vp.vz), prec, msg="Problem with %s derivative."%v)
                self.assertLess(abs(p_fd.m -vp.m ), prec, msg="Problem with %s derivative."%v)
