...
static void jacobi_to_inertial_posvel(struct reb_simulation* r){
    ...
    particles[0].vz = 0.0;
    for (int i=1;i<r->N;i++){
        particles[0].x  -= particles[i].m/particles[0].m*particles[i].x;
        particles[0].y  -= particles[i].m/particles[0].m*particles[i].y;
        particles[0].z  -= particles[i].m/particles[0].m*particles[i].z;
        particles[0].vx -= particles[i].m/particles[0].m*particles[i].vx;
        particles[0].vy -= particles[i].m/particles[0].m*particles[i].vy;
        particles[0].vz -= particles[i].m/particles[0].m*particles[i].vz;
    }
    for (int i=0;i<r->N;i++){
        particles[i].x  += ri_whfast512->p_jh0[0].x;
        particles[i].y  += ri_whfast512->p_jh0[0].y;
        particles[i].z  += ri_whfast512->p_jh0[0].z;
        particles[i].vx += ri_whfast512->p_jh0[0].vx;
...
// Performs one full interaction step
static void reb_whfast512_interaction_step_4planets(struct reb_simulation * r){
#ifdef PROF
    struct reb_timeval time_beginning;
    gettimeofday(&time_beginning,NULL);
#endif
...
static void reb_whfast512_interaction_step_2planets(struct reb_simulation * r){
#ifdef PROF
    struct reb_timeval time_beginning;
    gettimeofday(&time_beginning,NULL);
#endif
        ...
        case REB_WHFAST512_COORDINATES_DEMOCRATICHELIOCENTRIC:
            switch (N_systems){
                ...
                case 4:
                    ...
                    break;
                default:
                    reb_simulation_error(r,"Invalid N_systems");
            }
            for (int i=0;i<8;i++){
...
void reb_integrator_whfast512_part1(struct reb_simulation* const r){
    ...
    if (ri_whfast512->N_systems==1){
        ...
        reb_whfast512_interaction_step_4planets(r);
    }else if (ri_whfast512->N_systems==4){
        reb_whfast512_interaction_step_2planets(r);
    }
