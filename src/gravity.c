...
#pragma omp parallel for schedule(guided)
            ...
            break;
        case REB_GRAVITY_BRACE:
            switch (r->ri_brace.mode){
                case REB_BRACE_MODE_INTERACTION: // Interaction step
                {
                    double mtot = 0.0;
                    for (int i=0; i<_N_real; i++){
                ...
                break;
                case REB_BRACE_MODE_NONE: // In-between steps. Do not calculate anything. 
                    break;
                default:
                    reb_simulation_error(r, "BRACE mode not supported in gravity.c");
                    break;
            }
            break;
        default:
